...
def load_model(i_res, j_res, marker_density):
	image = np.load("test3.npy").astype(int)
...
def load_model_simple(i_res, j_res, marker_density):
	image = np.load("test2.npy").astype(int)
	image_i, image_j = image.shape
	...
	rho_key = np.array([3200,3300])
	eta_key = np.array([10**20,10**22])
	mu_key = np.array([10**10,10**10])
	C_key = np.array([10**7,10**7])
	sinphi_key = np.array([.6,.6])
